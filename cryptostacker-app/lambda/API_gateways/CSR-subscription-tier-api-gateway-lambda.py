...
def get_all_subscriptions(RDS_secret_host, RDS_secret_user, RDS_secret_pass):
    #http method GET
    logging.critical("get_all_subscriptions() called") #debugging
    limit_max = 200
    try:
        with connect(
...
def lambda_handler(event, context):
    ...
    if event["httpMethod"] == "GET":
        ...
        if "scope" in event["queryStringParameters"]:
            ...
            if event["queryStringParameters"]["scope"] == "allsubscriptions":
                logging.error("scope: allsubscriptions") #debugging
                sql_query_result = get_all_subscriptions(RDS_secret_host, RDS_secret_user, RDS_secret_pass)
